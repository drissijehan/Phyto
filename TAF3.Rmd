...
BasePK<-join(QPK,Surface,by=c("CODE_REG","ESPECE"))

BasePK$DosePK<-BasePK$quantite_pk/BasePK$Area
SommeDosePK<- aggregate(DosePK~PHYTOPROD+CODE_REG, data= BasePK, sum)
SommeDosePK <- ChangeNameCol(SommeDosePK,"DosePK","DosePKToutesCultures")

# visualisation de la disparité des dosesPK entre les différentes régions pour un AMM donné
minDosePKRegionaux <- aggregate(SommeDosePK$DosePK,by=list(AMM=SommeDosePK$PHYTOPROD),min)
minDosePKRegionaux <- ChangeNameCol(minDosePKRegionaux,"x","min")
maxDosePKRegionaux <- aggregate(SommeDosePK$DosePK,by=list(AMM=SommeDosePK$PHYTOPROD),max)
maxDosePKRegionaux <- ChangeNameCol(maxDosePKRegionaux,"x","max")
dispDosePKRegionaux <- merge(minDosePKRegionaux,maxDosePKRegionaux,by="AMM")
dispDosePKRegionaux$maxMinRatio <- dispDosePKRegionaux$max/dispDosePKRegionaux$min
hist(dispDosePKRegionaux$maxMinRatio,breaks=100)
#=> grosses disparités d'utilisation des produits sur l'ensemble des cultures suivant les régions d'après PK

BasePK<-merge(BasePK, SommeDosePK, by=c("PHYTOPROD","CODE_REG"),all.x=TRUE)
